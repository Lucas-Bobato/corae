...
export default function RestaurantsScreen() {
  ...
  const { totalReviews, averageCommunityRating } = useMemo(() => {
    ...
    restaurants.forEach((restaurant) => {
      const reviewsForRestaurant = getReviewsByRestaurant(Number(restaurant.id));
      reviewSum += reviewsForRestaurant.length;
      const restaurantAverage = getAverageRating(Number(restaurant.id));
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        console.error(err);
        setError("Não foi possível carregar a lista de restaurantes. Verifica se o backend está a rodar.");
      } finally {
        setLoading(false);
      }
    }
          ...
          <TouchableOpacity
            onPress={() => {
              ...
              getRestaurants()
                ...
                .catch(() => setError("Erro ao tentar novamente. Verifica se o backend está a rodar."))
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: "#F5F7F1" }}>
      ...
      {filteredRestaurants.length === 0 ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => {
            const averageRating = getAverageRating(Number(item.id));
            const reviewCount = getReviewsByRestaurant(Number(item.id)).length;
            return (
              <View className="px-4">
                <RestaurantCard 
                  {...item} 
